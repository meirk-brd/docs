...
If no synchronous subagents remain after that, Deep Agents does not add the `task` tool. This behavior only affects synchronous subagents. Async subagents still use the async task tools described in [Async subagents](/oss/javascript/deepagents/async-subagents).
